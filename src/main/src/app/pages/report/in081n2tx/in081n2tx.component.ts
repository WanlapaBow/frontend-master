...
import { defaultCofDef, logSetting } from '../in081n2tx/ag-gird.config';
import { In081n2txService } from './in081n2tx.service';

@Component({
  selector: 'ngx-in081n2tx',
  templateUrl: './in081n2tx.component.html',
  styleUrls: ['./in081n2tx.component.scss'],
})
export class In081n2txComponent implements OnInit, AfterViewInit {
  ...
  constructor(public commonService: CommonService,
    public service: In081n2txService,
  ...
  ngOnInit(): void {
    this.form = new FormGroup({
      ...
      statementStatus: new FormControl({ value: 'CONFIRMED', disabled: false }, Validators.required),
      flagReport: new FormControl({ value: 'pdf', disabled: false }, Validators.required),
    });
  ...
  exportReport() {
    ...
    this.service.getGenerate2(this.form.value.flagReport, this.form.value)
    ...
    dataLogDisplay.forEachNode((node, index) => {
      if (node.data.jobInfoStatus !== 'Failed' && node.data.jobInfoStatus !== 'ReportFailed' && node.data.jobInfoStatus !== 'Canceled' && node.data.jobInfoStatus !== 'Completed') {
        ...
        if (updateDataIndex >= 0) {
          ...
          node.data.successStatement = updateData[updateDataIndex].successStatement;
          node.data.endProcessing = updateData[updateDataIndex].endProcessing;
          node.data.startImportDataProcessing = updateData[updateDataIndex].startImportDataProcessing;
          node.data.startCreateStatementProcessing = updateData[updateDataIndex].startCreateStatementProcessing;
